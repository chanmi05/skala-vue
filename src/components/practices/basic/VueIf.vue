...
<template>
  <div class="practice-section">
    ...
    <button @click="isLogged = !isLogged">{{ isLogged ? '로그아웃' : '로그인' }}</button>
    <br />
    <h3>2) 성적별 학점 등급 측정(다중 조건문)</h3>
    <label>현재 점수 입력: </label>
    <input type="number" v-model="score" min="0" max="100" step="5" />
    <br />
    <div v-if="score >= 90" style="color: blue; font-weight: bold">
      합격 등급 : A학점입니다.(훌륭합니다!)
    </div>
    <div v-else-if="score >= 80" style="color: green">합격 등급 : B학점입니다.(양호합니다.)</div>
    <div v-else-if="score >= 70" style="color: orange">
      합격 등급 : C학점입니다.(조금 더 분발하세요.)
    </div>
    <div v-else style="color: red; font-weight: bold">
      합격 등급 : F학점입니다.(재시험 대상입니다.)
    </div>
  </div>
</template>
