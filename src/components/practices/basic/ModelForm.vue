...
const favoriteColors = ref([]) // 다중 체크박스는 반드시 배열([])로 시작!
const gender = ref('')
...
<template>
  <div class="pratice-section">
    ...
    <label><input type="checkbox" value="사과" v-model="favoriteColors" /> 사과</label> &nbsp;
    <label><input type="checkbox" value="바나나" v-model="favoriteColors" /> 바나나</label> &nbsp;
    <label><input type="checkbox" value="딸기" v-model="favoriteColors" /> 딸기</label> &nbsp;
    <p>
      데이터 상태(배열) : <span>{{ favoriteColors }}</span>
    </p>
