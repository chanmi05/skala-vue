...
const handleFetchWeather = async () => {
  ...
  const API_KEY = '650a0eba19d52bdf22765adc56eb21db'
  const URL = `https://api.openweathermap.org/data/2.5/weather?lat=35.158582&lon=126.804975&appid=${API_KEY}&units=metric&lang=kr`

  try {
    // 비동기 통신 가동 : 서버에서 데이터를 다 가져올 때까지 await로 기다리기
    const response = await axios.get(URL)
    // fetch와 달리 .json() 변환 과정 없이 response.data에 알맹이가 즉시 담김.
    console.log(`Axios 통신 응답 객체: `, response)
