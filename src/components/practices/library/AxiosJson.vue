...
const handleCreate = async () => {
  ...
  try {
    ...
    // POST 함수 구조 : (주소, 보낼 데이터)
    const respone = await axios.post(BASE_URL, payload)
    console.log('POST 성공', respone.data)

    // 응답 데이터를 배열 맨 앞에 추가하여 화면에 즉시 반영
    items.value.unshift(respone.data)
...
<template>
  <div class="practice-section">
    ...
    <div class="input-zone">
      <input v-model="textInput" placeholder="저장할 텍스트를 입력해주세요" />
